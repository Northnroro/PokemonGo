$(function() {
  function inIframe () {
	...
	if (inIframe()) {
        ...
        type: "GET",
        url: "/sec",
        data: "authorize=" + Math.random() + ".fpmSecretToken",
        success: function (dt, status, request) {
            var serverDate = new Date(request.getResponseHeader("date"));
            var localDate = new Date();
            window.offset = serverDate.getTime() - localDate.getTime();
            console.log(parseInt(offset/1000) + "second interval with server time")
            window.odate = Date.now;
            Date.now = function() {
              return window.odate()+window.offset;
            };
        }
    });
...
window.alert = function(msg) {console.log(msg);return false;}
...
L.HtmlIcon = L.Icon.extend({
    ...
    createIcon: function() {
        ...
        if(this.options.hide) {
            ...
            div.innerHTML =
                '<div onclick="openInNavigation(this, event)" style="cursor: pointer" class="displaypokemon" data-lat="'+this.options.latitude+'" data-lng="'+this.options.longitude+'" data-pokeid="' + this.options.pokemonid  + '">' +
                '<div class="pokeimg">' +
                '<img src="data:image/png;base64,' + pokemonPNG[this.options.pokemonid] + '" />' +
                '</div>' +
                '<div class="remainingtext" data-expire="' + this.options.expire + '"></div>' +
...
function openInNavigation(element, event) {
    event.stopPropagation();
    event.preventDefault();
    if (map.getZoom() >= 16) {
        var lat = $(element).attr('data-lat');
        var lng = $(element).attr('data-lng');
        var open = confirm('Do you want to open navigation to this pokemon?');
        if (open == true) {
...
function createPokeIcon(pokemonid, timestamp, filtered, lat, lng) {
...
function loadCache(cp) { //Loads cache around object
    $.getJSON("https://cache.fastpokemap.se/?lat=" + cp.lat + "&lng=" + cp.lng, function(data) {
        ...
        if (data.length >= 1) {
            ...
            for (i in data) {
				...
				if ( new Date(cachedSpawn.expireAt).getTime() > Date.now() ) {
...
function addPokemonToMap(spawn) {
    ...
    if (toAdd) {
        ...
        var pokeMarker = new L.marker(cp, {
            icon: createPokeIcon(pokeid, spawn.expiration_timestamp_ms, filtered, spawn.latitude, spawn.longitude)
        });
        shownMarker.push({
            marker: pokeMarker,
...
$(function() {    
    ...
    $('.nearby').on('click', toggleView);
    if (window.client != "FPM-Client") {
      	   $('#menu').slicknav({ label: '' });
    ...
    $.get("pokedata", function(data) { //Init map
